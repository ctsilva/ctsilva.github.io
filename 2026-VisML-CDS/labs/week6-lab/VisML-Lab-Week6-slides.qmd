...
subtitle: "DS-GA 3001 - Fall 2026 - Interpreting Black Box Models"
...
::: {.notes}
Today’s lab is all about understanding black box models — models that make accurate predictions but are hard to interpret. We’ll focus on two popular techniques, LIME and SHAP, to explain model predictions. If time allows, we’ll also look at Principal Component Analysis, and next week we’ll move into visualizing deep learning models.
:::
...
::: {.notes}
When we talk about black box models, we mean models like random forests or neural networks where it’s hard to understand why they make certain predictions. Unlike white box models, they don’t offer simple rules or equations. But we do have techniques to peek inside — tools like LIME and SHAP help us understand which features are influencing the model’s decisions.
:::

## Local Interpretable Model-Agnostic Explanations (LIME)

* What is LIME?
  * LIME is a method that explains the prediction of any classifier by learning an interpretable model locally around the prediction
::: {.notes}
LIME, or Local Interpretable Model-Agnostic Explanations, is a method for understanding individual predictions made by complex models. The key idea is that instead of trying to explain the entire model, LIME focuses locally — it builds a simple, interpretable model around one specific prediction to show which features influenced it the most.
:::
...
::: {.notes}
LIME is useful because it balances interpretability and flexibility. It’s easy for non-experts to understand since it explains predictions with a simple, local model. It also maintains local fidelity, meaning it closely mimics the black box model’s behavior near the instance of interest. Plus, it’s model-agnostic, so it works with any classifier. And when applied across many samples, it can even give a global sense of how the model behaves overall.
:::
...
::: {.notes}
We start with our original model f, and an instance x that we want to explain. LIME generates many perturbed samples around x, called z', and uses a proximity function πₓ to assign higher weights to samples that are closer to x—ensuring we stay local.
Next, we train an interpretable model g—like a linear model or a shallow decision tree—on this neighborhood. The goal is for g to mimic the behavior of f around x, while remaining easy to understand.
This process is captured in the equation shown:

Here:
L(f,g,πx)\mathcal{L}(f, g, \pi_x)L(f,g,πx​) is the local fidelity loss, which measures how well g approximates f near x, weighted by the proximity function πₓ.


Ω(g)\Omega(g)Ω(g) is a complexity penalty on the explanation model g, encouraging simpler explanations—for example, by penalizing deeper trees or more complex models.


So, LIME is all about balancing two objectives:
Fidelity: staying true to the original model's predictions locally, and


Interpretability: keeping the explanation simple enough for humans to understand.
:::
...
::: {.notes}
SHAP is based on the concept of Shapley values from game theory — it treats each feature as a player contributing to the final prediction. The idea is to fairly distribute credit among features for how much each one influences the output. In this setup, the game is the prediction task, and the gain is how much the prediction differs from the average. SHAP is powerful for debugging models and explaining individual predictions in a consistent, mathematically grounded way.
:::
...
::: {.notes}
Traditional feature importance tells us which features matter most overall, but it misses key details. It doesn’t explain how a feature affects individual predictions, whether it tends to increase or decrease the output, or how it influences class probabilities in classification tasks. That’s where SHAP comes in — it fills these gaps by showing both the direction and magnitude of each feature’s impact.
:::
...
::: {.notes}
This waterfall plot shows how SHAP values explain a single prediction — in this case, the predicted bonus for an employee. We start at the average predicted bonus, shown on the left, and then each feature pushes the prediction up or down. For example, having a degree increases the predicted bonus by about $16.9, while sales slightly decreases it. The final prediction on the right combines all these contributions, giving us a transparent view of how each feature influenced the outcome.
:::
...
``` {python}
class_idx = 0  # or 1, depending on which class you want to explain
...
shap_values_class_0 = shap_values[:, :, 0]  # Get SHAP values for class 0

shap.dependence_plot(feature_name, shap_values_class_0, X_test, interaction_index=interaction)
...
shap_values_class_0 = shap_values[:, :, 0]  # shape: (20, 4)

# Now everything matches in shape
shap.initjs()
shap.force_plot(explainer2.expected_value[0], shap_values_class_0[sample_idx], X_test.iloc[sample_idx])
...
sample_idx = 10        # choose a valid sample index
class_idx = 1          # class index (0 or 1 for binary classification)

# Slice SHAP values correctly: get SHAP values for the specified class
shap_values_for_class = shap_values[:, :, class_idx]  # shape: (20, 4)

# Now plot for a single sample
shap.force_plot(
    explainer2.expected_value[class_idx],         # base value for class 1
    shap_values_for_class[sample_idx],            # SHAP values for that sample
    X_test.iloc[sample_idx]                       # feature values for that sample
)
```
